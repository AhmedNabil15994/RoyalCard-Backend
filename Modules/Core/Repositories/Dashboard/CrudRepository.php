...
class CrudRepository
{
    ...
    public function getAllActive($order = 'id', $sort = 'desc',$with=[])
    {
        $record = $this->model->with($with)->active()->orderBy($order, $sort)->get();
        return $record;
    }
    ...
    public function QueryTable($request)
    {
        $query = $this->model->where(function ($query) use ($request) {
            ...
            foreach ($this->getModelTranslatable() as $key) {
                $col = $key . '->' . locale();
                $query->orWhere(DB::raw('LOWER(' . $key . ')'), 'like', '%' . $request->input('search.value') . '%');
            }
        });
