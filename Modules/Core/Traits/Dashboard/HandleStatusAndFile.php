...
trait HandleStatusAndFile
{
     ...
     * @return void
     */
    public function handleFileAttributeInRequest($model, Request $request, bool $deleteBeforeUpdate = false,$keep = null): void
    {

        if ($model instanceof HasMedia) {
            $this->handleFileIfMediaImplement($model, $request, $deleteBeforeUpdate,$keep);
        } else {
     ...
     * @return void
     */
    public function handleFileIfMediaImplement($model, Request $request, bool $deleteBeforeUpdate = false,$keep = null): void
    {
        foreach ($this->fileAttribute as $file => $collection) {


            if ($request->$file) {
                if ($deleteBeforeUpdate == true && !$keep || ($keep != $file)) {
