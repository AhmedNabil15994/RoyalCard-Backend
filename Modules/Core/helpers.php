...
if (!function_exists('getPaymentGateway')) {
    function getPaymentGateway($country_id)
    {
        $gateways = setting('payment_gateway');
        foreach ($gateways as $key => $value) {
            if(isset($value['country_id']) && $value['country_id'] == $country_id) {
                return $key;
            }
        }
    }
...
if (!function_exists('generateRandomOTPCode')) {
    function generateRandomOTPCode($length = 5)
    {
